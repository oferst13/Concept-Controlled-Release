...
def f(X):
    ...
    for i in range(sim_len):
        ...
        outlet_A[:, i, 0] = (((overflows[:, i] + (releases[:, i] * dt)) / dt) / tank_alphas) ** (1 / beta)
    ...
    plt.legend(line_objects, ('Pipe 1 - outflow', 'Pipe 2 - outflow', 'Pipe 3 - outflow', 'Pipe 1 - inflow', \
                              ...
                              '''
    mass_balance_err = 100 * (abs(integrate.simps(pipe_Q[2, :, 1] * dt, t[0:-1])-(np.sum(overflows)+(np.sum(releases)*dt))) /
                              (np.sum(overflows) + (np.sum(releases) * dt)))
