...
def forecast():
    timesteps = int(forecast_horizon / dt)
    if i+timesteps > sim_len:
        inflows = np.sum(rain_volume[i:], axis=1)
    else:
        inflows = np.sum(rain_volume[i:i+timesteps], axis=1)
    return inflows


forecast_horizon = 2 * 60 * 60
dt = 30
rain_dt = 600
beta = 5 / 4
manning = 0.012
...
release_alphas = np.linspace(0, 0.9, 10)
to_min = 0
penalty = 0
runs = 0
...
for i in range(sim_len):
    ...
    if i <= bm.last_overflow:
        if i % (forecast_horizon / dt) == 0:
            release_deg = forecast()
    else:
        release_deg = 0.0
